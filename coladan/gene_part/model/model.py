...
class TransformerModel(nn.Module):
    ...
    def transformer_generate(
        ...
        coconut_input: Optional[Tensor] = None,
        ...
        if coconut_input is not None:
            gen_total_embs = coconut_input
        
        if (coconut_input is None) and (attn_non_mask_position is None):
    ...
    def generative_forward(
        ...
        output = {}
        output["transformer_gen_output"] = gen_output
        ...
        if self.explicit_zero_prob and do_sample:
            ...
            pred_sigma = decoder_output["pred_sigma"]
            output["pcpt_preds"] = full_preds[:, : pcpt_genes.shape[1]]
            output["pcpt_preds_sigma"] = pred_sigma[:, : pcpt_genes.shape[1]]
            output["gen_preds"] = full_preds[:, pcpt_genes.shape[1] :]
            output["gen_preds_sigma"] = pred_sigma[:, pcpt_genes.shape[1] :]
        if self.explicit_zero_prob:
...
class ExprDecoder(nn.Module):
    def __init__(
        ...
        self.head_logsigma = nn.Linear(d_model, 1)
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        h_mean = self.mean(x) # [batch, seq_len, d_hidden]
        h_sigma = self.sigma(x) # [batch, seq_len, d_hidden]
        pred_value = self.head_mean(h_mean).squeeze(-1)  # (batch, seq_len)
        pred_sigma = self.head_logsigma(h_sigma).squeeze(-1)

        if not self.explicit_zero_prob:
            return dict(pred=pred_value,pred_sigma=pred_sigma)
        zero_logits = self.zero_logit(x).squeeze(-1)  # (batch, seq_len)
        zero_probs = torch.sigmoid(zero_logits)
        return dict(pred=pred_value, pred_sigma=pred_sigma,zero_probs=zero_probs)

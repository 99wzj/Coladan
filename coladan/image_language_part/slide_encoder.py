...
class LongNetViT(nn.Module):
    ...
    def coords_to_pos(self, coords, tile_size: int = 1):
        """
    ...
    def forward(self, x, coords, all_layer_embed=False,tile_size = 1):
        """
        ...
        pos = self.coords_to_pos(coords, tile_size)  # [N, L]
        ...
        #notice only for 1 slide per time
        outcomes = []
        assert len(x_list) == 1
        for x in x_list:
            if self.global_pool:
...
#new create_model for Coladan register
def create_model(model_arch: str, in_chans: int, **kwargs):
    model = timm.create_model(model_arch, pretrained=False, in_chans=in_chans, **kwargs)
    return model
